...
function updateTransactionsList() {
	var update = setInterval(function(){
		$.getJSON("includes/ajax.transactions.php",function(transaction) {
...
function sortTable(elem_selector,col_num,desc){
	...
	rows.sort(function(a, b) {
		...
		var A = parseFloat($(a).children('td').eq(col_num).text().replace('$','').replace(',',''));
		var B = parseFloat($(b).children('td').eq(col_num).text().replace('$','').replace(',',''));
		A = (isNaN(A)) ? 0 : A;
		B = (isNaN(B)) ? 0 : B;
