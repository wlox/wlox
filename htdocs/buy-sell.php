...
if ($_REQUEST['buy']) {
	...
	if (!is_array(Errors::$errors) && !$cancel) {
		if ($confirmed) {
			$operations = Orders::executeOrder(1,$buy_price1,$buy_amount1,$currency1,$user_fee['fee'],$buy_market_price1);
...
if ($_REQUEST['sell']) {
	...
	if (!is_array(Errors::$errors) && !$cancel) {
		if ($confirmed) {
			$operations = Orders::executeOrder(0,$sell_price1,$sell_amount1,$currency1,$user_fee['fee'],$sell_market_price1);
			if ($operations['new_order'] > 0) {
				Link::redirect('open-orders.php',array('transactions'=>$operations['transactions'],'new_order'=>1));
				exit;
			}
...
if ($ask_confirm && $_REQUEST['sell']) {
	$bank_accounts = BankAccounts::get(User::$info['id'],$currency_info['id']);
	if (!$bank_accounts)
